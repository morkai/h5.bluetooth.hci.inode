...
exports.deviceModelDecoders = {
  ...
  [DeviceModel.EnergyMeter]: function(buffer, eirDataStructure)
  {
    ...
    decodeRtto(buffer, 0, eirDataStructure);
    decodeAlarms(buffer, 0, -1, eirDataStructure);
    decodeEnergyMeter(buffer, 2, eirDataStructure);
  },
 ...
 * @private
 * @param {Buffer} buffer
 * @param {?function(Buffer, number, Object): void} decodePosition
 * @param {?function(Buffer, number, Object): void} decodeTemperature
 * @param {?function(Buffer, number, Object): void} decodeHumidity
 * @param {INodeCareSensorMsd} eirDataStructure
 */
function decodeCareSensor(buffer, decodePosition, decodeTemperature, decodeHumidity, eirDataStructure)
{
  ...
  decodeBatteryLevel(buffer, 2, 12, eirDataStructure);
 ...
 * @param {Buffer} buffer
 * @param {number} i
 * @param {(INodeCareSensorMsd|INodeCareRelayMsd)} eirDataStructure
 */
 ...
 * @param {Buffer} buffer
 * @param {number} i
 * @param {INodeDeviceMsd} eirDataStructure
 */
 ...
 * @param {number} i
 * @param {number} shift
 * @param {(INodeCareSensorMsd|INodeEnergyMeterMsd)} eirDataStructure
 */
function decodeBatteryLevel(buffer, i, shift, eirDataStructure)
{
 ...
 * @param {Buffer} buffer
 * @param {number} i
 * @param {INodeEnergyMeterMsd} eirDataStructure
 */
function decodeEnergyMeter(buffer, i, eirDataStructure)
{
  ...
  eirDataStructure.unit = unit;
  eirDataStructure.constant = constant;
  eirDataStructure.average = Math.round(60 * buffer.readUInt16LE(i) / constant * 1000) / 1000;
  eirDataStructure.sum = Math.round(buffer.readUInt32LE(i + 2) / constant * 1000) / 1000;
  ...
  {
    decodeBatteryLevel(buffer, i + 8, 4, eirDataStructure);

    eirDataStructure.lightLevel = Math.round((buffer[i + 8] & 0x0F) * 100 / 15 * 10) / 10;

    const weekDataData = buffer.readUInt16LE(i + 9);

    eirDataStructure.weekDay = weekDataData >> 13;
    eirDataStructure.weekDayTotal = weekDataData & 0x1FFF;
  }
  else
  {
    eirDataStructure.batteryLevel = 100;
    eirDataStructure.batteryVoltage = 2.88;
    eirDataStructure.lightLevel = 0;
    eirDataStructure.weekDay = new Date(Date.now() - 24 * 3600 * 1000).getDay();
    eirDataStructure.weekDayTotal = 0;
  }
}
